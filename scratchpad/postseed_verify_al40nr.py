# -*- coding: utf-8 -*-
"""POST-SEED verification for AL_FORM_40NR.
...
check(n == BASELINE_FORMS + 1, f"prod moved {BASELINE_FORMS} -> {n} forms (exactly +1)",
      f"prod is at {n} forms, expected {BASELINE_FORMS + 1}")
...
snap = json.load(open(os.path.join(PROJECT_ROOT, "scratchpad",
                                   "preflight_seed_al40nr_snapshot.json"), encoding="utf-8"))
diffs = 0
